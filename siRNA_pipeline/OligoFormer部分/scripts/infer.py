...
def calculate_td(df):
	...
	for i in range(df.shape[0]):
		...
		df.loc[i, 'DG_1'] = DeltaG[df.iloc[i,0][0:2]]
		df.loc[i, 'DH_1'] = DeltaH[df.iloc[i,0][0:2]]
		df.loc[i, 'U_1'] = int(df.iloc[i,0][0] == 'U')
		df.loc[i, 'G_1'] = int(df.iloc[i,0][0] == 'G')
		df.loc[i, 'DH_all'] = Calculate_DGH(df.iloc[i,0])[1]
		df.loc[i, 'U_all'] = df.iloc[i,0].count('U') / 19
		df.loc[i, 'UU_1'] = int(df.iloc[i,0][0:2] == 'UU')
		df.loc[i, 'G_all'] = df.iloc[i,0].count('G') / 19
		df.loc[i, 'GG_1'] = int(df.iloc[i,0][0:2] == 'GG')
		df.loc[i, 'GC_1'] = int(df.iloc[i,0][0:2] == 'GC')
		df.loc[i, 'GG_all'] = [df.iloc[i,0][j]+df.iloc[i,0][j+1] for j in range(18)].count('GG') / 18
		df.loc[i, 'DG_2'] = DeltaG[df.iloc[i,0][1:3]]
		df.loc[i, 'UA_all'] = [df.iloc[i,0][j]+df.iloc[i,0][j+1] for j in range(18)].count('UA') / 18
		df.loc[i, 'U_2'] = int(df.iloc[i,0][1] == 'U')
		df.loc[i, 'C_1'] = int(df.iloc[i,0][0] == 'C')
		df.loc[i, 'CC_all'] = [df.iloc[i,0][j]+df.iloc[i,0][j+1] for j in range(18)].count('CC') / 18
		df.loc[i, 'DG_18'] = DeltaG[df.iloc[i,0][17:19]]
		df.loc[i, 'CC_1'] = int(df.iloc[i,0][0:2] == 'CC')
		df.loc[i, 'GC_all'] = [df.iloc[i,0][j]+df.iloc[i,0][j+1] for j in range(18)].count('GC') / 18
		df.loc[i, 'CG_1'] = int(df.iloc[i,0][0:2] == 'CG')
		df.loc[i, 'DG_13'] = DeltaG[df.iloc[i,0][12:14]]
		df.loc[i, 'UU_all'] = [df.iloc[i,0][j]+df.iloc[i,0][j+1] for j in range(18)].count('UU') / 18
		df.loc[i, 'A_19'] = int(df.iloc[i,0][18] == 'A')
...
def infer(Args):
	...
	if Args.infer == 1:
		...
		for _name, _mRNA in fa_dict.items():
			...
			Y_PRED = [i*1.341 for i in Y_PRED]#乘了1.341，是为了和pita(预测RNA靶点的工具)的score保持一致,为了后面进行效果比较
